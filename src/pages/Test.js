...
const Test = () => {
  ...
  const reducer = (state, action) => {
    switch (action.type) {
      case "INCREMENT":
        return { numbere: state.numbere + 1, text: state.text };
      case "DECREMENT":
        return { numbere: state.numbere - 1, text: state.text };
      case "toggler":
        return { numbere: state.numbere, text: !state.text };
      default:
        return state;
    }
  };
  ...
  const [state, dispatch] = useReducer(reducer, { numbere: 0, text: true });
